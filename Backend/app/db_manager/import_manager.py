from app.db_manager.database import Database

class ImportManager:
    def __init__(self):
        self.db = Database()

    async def insert_import(self, file_id: str, import_type: str, source: str, modules: list, aliases: list = None):

        query = """
        INSERT INTO imports (file_id, type, source, modules, aliases)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id;
        """
        return await self.db.fetchrow(query, file_id, import_type, source, modules, aliases)

    async def get_imports_by_file_id(self, file_id: str):
        query = "SELECT id, file_id, type, source, modules, aliases FROM imports WHERE file_id = $1;"
        return await self.db.fetch(query, file_id)

    async def delete_imports_by_file_id(self, file_id: str):
        query = "DELETE FROM imports WHERE file_id = $1;"
        await self.db.execute(query, file_id)

    async def copy_import_table(self, data: list, columns: list):
        await self.db.copy_to_table('imports', data, columns=columns)

    async def copy_import_modules_table(self, data: list, columns: list):
        await self.db.copy_to_table('import_modules', data, columns=columns)